...
class BOKEH_PLOT(object):
    ...
    def figure(self, show = True, debug=0):
        ...
        self.TOOLS="crosshair, pan, wheel_zoom, box_zoom, reset, save"    # Tools for Bokeh
        # box_select
    ...
    def ylim(self, ymin, ymax, debug=0):          # y limits for the plot
        ...
        '''
        if debug >1: print("defining ymin: {0} and ymax:{1} ".format(ymin, ymax))
        self.ymax = ymax
    ...
    def savefig(self, namehtml, debug=0):
        ...
        '''
        if debug>0: print("### Bokeh savefig as {0} ".format(namehtml))
        if self.showed:
