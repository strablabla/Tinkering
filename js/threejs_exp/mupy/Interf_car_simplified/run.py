...
try:
    ser = serial.Serial('/dev/rfcomm0', 115200, timeout=1) # Establish serial connection. 
except:
...
def direction(message):
    ...
    emit('my response',
         {'data': message['data'], 'count': session['receive_count']})
    try:
        ser.write(dic[str(message['data'])]) # sending command to car.
    except:
        print("couldn't send command to the car")
