    ...
    function render(dt) {
      ...
      if (shoot){
         ...
         for (i=1;i<listballs.length;i++){
           ...
           var point2 = listballs[i].position.clone();
           var distance = point1.distanceTo( point2 );
           if (distance < sizeball){
               listballs[i].material.color.setHex(0x111111)
        ...
        //bulletLight.position.set( camera.position.add(newdir) )
        //var newpos = camera.position.add(direction)
        //bulletLight.position.set( camera.position + direction )
        bulletLight.position.set( camera.position.x+direction.x, camera.position.y+direction.y, camera.position.z+direction.z )
        var factgun = 2
        gun.position.set( camera.position.x + direction.x/factgun, camera.position.y + direction.y/factgun, camera.position.z + direction.z/factgun )
