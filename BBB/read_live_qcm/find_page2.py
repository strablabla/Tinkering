...
cap.set(cv2.cv.CV_CAP_PROP_FRAME_WIDTH,320)# 320
cap.set(cv2.cv.CV_CAP_PROP_FRAME_HEIGHT,480)# 240
thresh = 5
max_thresh = 255
...
def new_dim(corners, ii):
    tl,tr,br,bl = corners
    print("points are {0}, {1}, {2}, {3} ".format(tl,tr,br,bl))
    ...
    ii.maxWidth, ii.maxHeight = maxWidth, maxHeight
    print('ii.maxWidth ', ii.maxWidth)
...
def perspective(ii, dst):
    ...
    im_th3 = cv2.resize(im_th3,(900,400),interpolation = cv2.INTER_AREA)
    ii.im_th2 = im_th2
    ii.im_th1 = im_th1
    ii.im_th3 = im_th3
...
def find_frame(ii):
    ...
    if len(approx) == 4:
        ...
        # pts_frame = [xmin, xmax, ymin, ymax]
        try:
            corners = order_pts_frame(screenCnt)
            print('helllllllllllloooooo')
            # print(tl,tr,br,bl)
        except:
            pass
        try:
            print('trying new dim')
            dst = new_dim(corners,ii)
        except:
            pass
        try:
            perspective(ii, dst)
